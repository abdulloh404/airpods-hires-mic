...
use airpods_core::dsp::{MIC_GAIN_DB, MIC_LIMIT_DBFS, validate_mic_settings};
use bluer::Address;
...
#[tauri::command]
pub fn save_device_address(address: String) -> Result<String, String> {
    ...
    let mut lines: Vec<&str> = contents
        .lines()
        .filter(|line| !line.starts_with("AIRPODS_DEVICE="))
        .collect();
    while lines.last().is_some_and(|line| line.is_empty()) {
        lines.pop();
    }
    ...
    updated.push_str(&format!("AIRPODS_DEVICE={address}\n"));
    write_atomic(&path, updated.as_bytes())?;
    Ok(address)
}
